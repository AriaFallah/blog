...
reason this is an error is because python is strongly typed. `add(5,
"1.2313213")` would be 100% valid in a weakly typed language like JavaScript.

Note that

* The code is more concise
* There is no type inference going on. In dynamically typed languages variables
  are just containers for values, and have no other special properties. `add(x,
  s)` fails because during run-time you try to add an `int` and a `string` not
  because the interpreter figured out in advance `x` and `s` are not compatible.
